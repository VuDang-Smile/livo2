...
#include <Eigen/Dense>
#include "standalone_stubs.hpp"
#ifndef STANDALONE_HBA
#include <ros/ros.h>
...
int main(int argc, char** argv)
{
  ...
  if (argc < 4) {
    cout << "Usage: hba_standalone <data_path> <total_layer_num> <thread_num> [pcd_name_fill_num]" << endl;
    return 1;
  }
  string data_path = argv[1];
  int total_layer_num = atoi(argv[2]);
  int thread_num = atoi(argv[3]);
  if (argc > 4) pcd_name_fill_num = atoi(argv[4]);
