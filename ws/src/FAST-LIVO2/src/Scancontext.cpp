...
float xy2theta( const float & _x, const float & _y )
{
    ...
    // Default case (should not reach here, but added to avoid compiler warning)
    return 0.0f;
} // xy2theta
...
bool SCManager::saveScancontextToFile(const std::string& file_path, const Eigen::MatrixXd& sc, 
                                       const Eigen::MatrixXd& ringkey, const Eigen::MatrixXd& sectorkey)
{
    try {
        ...
        // Write ringkey
        int ringkey_size = ringkey.rows();
        out_file.write(reinterpret_cast<const char*>(&ringkey_size), sizeof(int));
        for (int i = 0; i < ringkey_size; i++) {
            double value = ringkey(i, 0);
            out_file.write(reinterpret_cast<const char*>(&value), sizeof(double));
        }
        
        // Write sectorkey
        int sectorkey_size = sectorkey.cols();
        out_file.write(reinterpret_cast<const char*>(&sectorkey_size), sizeof(int));
        for (int i = 0; i < sectorkey_size; i++) {
            double value = sectorkey(0, i);
            out_file.write(reinterpret_cast<const char*>(&value), sizeof(double));
        }
        ...
        std::cerr << "Error saving scancontext to file: " << e.what() << std::endl;
        return false;
    }
}
