...
void ThetaDriver::publishImage(GstMapInfo map) {
    ...
    if (fps_limit_ > 0.0) {
        ...
        double time_since_last_publish = (current_time - last_publish_time_).seconds();
...
void ThetaDriver::onInit() {
    ...
    if (fps_limit_ > 0.0) {
        min_publish_interval_ = 1.0 / fps_limit_;
        RCLCPP_INFO(get_logger(), "FPS limiting enabled: %.1f FPS (%.3f seconds between frames)", 
                   fps_limit_, min_publish_interval_);
    } else {
        min_publish_interval_ = 0.0;
